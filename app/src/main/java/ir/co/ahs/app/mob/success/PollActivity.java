...
public class PollActivity extends Activity{
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        toolbar_main_tv_header.setText("نظرسنجی");
        ...
        btnSubmit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Toast.makeText(PollActivity.this, "نظر شما با موفقیت ثبت شد", Toast.LENGTH_SHORT).show();
            }
        });

        // Dialog Add To Cart

        dialogPoll = new Dialog(PollActivity.this);
            ...
            public void onClick(View view) {
                ...
                if (isSelected) {
                    if (itemIDs.size() > 0) {
                        ...
                        Toast.makeText(PollActivity.this, "لطفا یک مورد را انتخاب کنید", Toast.LENGTH_SHORT).show();
                ...
                new AsyncPoll().execute(Urls.BASE_URL + Urls.SEND_POLL_ANSWER, sessionId, userToken, customerId, String.valueOf(A));
...
//                        Toast.makeText(PollActivity.this, item.getString("ItemID"), Toast.LENGTH_SHORT).show();
                    } else {
                        items = new JSONArray(itemsJSON);
                        JSONObject item = items.getJSONObject(v.getId());
                        itemID = item.getString("ItemID");
//                        Toast.makeText(PollActivity.this, item.getString("ItemID"), Toast.LENGTH_SHORT).show();
        ...
        adapter_recycler = new Adapter_Recycler(getApplicationContext(), arrayList, new OnItemListener() {
            ...
            public void onCardClick(int position) {
                dialogPoll.show();
                dialogPoll_Title.setText(arrayList.get(position).strText);
...
//                            struct.isDeletedItem = item.getBoolean("isDeletedItem");
    ...
    private void getPoll() {
        ...
        String userToken = G.AUTHENTICATIONS_TOKEN.getString("TOKEN", "x");
        String customerId = G.CUSTOMER_ID.getString("CUSTOMER_ID", "x");
        new Async().execute(Urls.BASE_URL + Urls.GET_POLL, sessionId, userToken, customerId);

    }

    private class Async extends Webservice.getPoll {
        @Override
...
//                    }
    ...
    private class AsyncPoll extends Webservice.sendPoll {
        @Override
        protected void onPreExecute() {
